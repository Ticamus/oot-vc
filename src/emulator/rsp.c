...
#if IS_MM
// MM-only: no-op stub used by soundRVL's not-yet-decompiled audio driver code.
void fn_80054B60(void) {
}

// MM time-slices display-list parsing: an OSAlarm handler clears this flag to
// make rspParseGBI hand control back, in place of OoT's explicit command count.
static s32 sRspTimeSliceActive = 1;
...
// fn_80054B10 below.
static void fn_80054AF0(OSAlarm* pAlarm, OSContext* pContext) {
    sRspTimeSliceActive = 0;
    fn_800988E8(lbl_80180660 + 0x31C, 1, 0);
}

// MM-only: per-RSP alarm handler; clears the "alarm armed" flag so
// fn_80054B64 can re-arm it next time it is needed.
static void fn_80054B10(OSAlarm* pAlarm, OSContext* pContext) {
    lbl_801FFF5C = 0;
}
